...
class ResBlock(nn.Module):
    def __init__(self, in_channel, out_channel, filter=False):
        ...
        self.dyna_ch = DAC(in_channel, kernel_size=3, num_groups=4) if filter else nn.Identity()

        self.sfconv = HEM(in_channel) if filter else nn.Identity()
    ...
    def forward(self, x):
        ...
        out = self.proj(out)  # DWConv
        out = self.proj_act(out)  # GELU
